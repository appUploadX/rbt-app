...
export class BcbaCalendarModalDetailsPage implements OnInit {
	...
	ngOnInit() {
      ...
      var newTime = time;
      var newYearmonth = yrmonth;
			console.log(link)
			$.ajax({
				url: link,
				type: 'POST',
				dataType: 'json',
				data: {action: 'cancel', datacode: code, note: msg,newTime:newTime,newYearmonth:newYearmonth},
	...
	approve(val, code)
	{
      ...
      var newYearmonth = this.yrmonth;
			$.ajax({
				url: link,
				type: 'POST',
				dataType: 'json',
				data: {datacode: code, type: val,newTime:newTime},
				success: function(data){
					console.log(data)
					if(data.status == 'ok')
					{
